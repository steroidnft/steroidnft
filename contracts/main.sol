...
contract Child is ERC721URIStorage {
    ...
    mapping(uint=>mapping(address=>uint)) ticketLevel;
    mapping(uint=>address) eventOrganiserAddress;
    mapping(uint=>uint) eventTicketPrice;
    mapping(uint=>uint) eventEndTime;
    mapping(uint=>uint) eventMoneyCollection;
    mapping(uint=>bool) canExtractEventMoney;
    ...
    function buyEventTicket(uint _eventId,string memory _name) external payable{
        ...
        if(msg.value>=(1.6 * eventTicketPrice[_eventId])){
            ticketLevel[_eventId][msg.sender]=3;
        }else if(msg.value>=(1.3 * eventTicketPrice[_eventId])){
            ticketLevel[_eventId][msg.sender]=2;
        }else{
            ticketLevel[_eventId][msg.sender]=1;
        }
        ...
        eventMoneyCollection[_eventId]+=msg.value;
    ...
    function getTicketLevel(uint _eventId,address addr) external view returns(bool,uint){
        bool ownTicket=haveTicket[_eventId][addr];
        uint ticketLevel=ticketLevel[_eventId][addr];
        returns(ownTicket,ticketLevel);
    }
